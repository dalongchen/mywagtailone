...
def my_bag(db_path):
    ...
    # print(y_test)
    # f = "my_bag_sun2"  # 子算法2
    f = "random_forest_classifier"  # 随机森林
    # f = "my_cross_value_score"  # 交叉验证法1
    # f = "my_grid_search_cv"  # 交叉验证法, 网格搜索优化参数
    if f == "my_bag_sun1":
        my_bag_sun1(x_test, x_train, y_test, y_train)        # 子算法1
    elif f == "my_bag_sun2":
        my_bag_sun2(x_test, x_train, y_test, y_train)        # 子算法2
    elif f == "my_cross_value_score":
        my_cross_value_score(db_path)  # 交叉验证法1
    elif f == "my_grid_search_cv":
        my_grid_search_cv(db_path)  # 交叉验证法, 网格搜索优化参数
    elif f == "random_forest_classifier":
        random_forest_classifier(db_path)  # 随机森林
...
# 随机采样参数优化
def randomized_search_cv(db_path):
    from sklearn.ensemble import RandomForestClassifier
    from sklearn.model_selection import RandomizedSearchCV, GridSearchCV  # 导入
    from scipy.stats import randint as sp_randint  # 生成a-b随机数
    from time import time
    x, y = load_data(db_path, f="my_cross_value_score")  # 加载元数据 dimensionless
    """
    ...
    #                "max_features": [1, 3, 10],
    #                "bootstrap": [True, False], }
    # grid_research = GridSearchCV(clf, param_grid=param_dist1)
    # grid_research.fit(x, y)
    # print ("Grid took %.2f seconds for %d candidates parameter settings"
    #        % (time()-start, len(grid_research.cv_results_['params'])))
    # report(grid_research.cv_results_)
    """random took 29.85 seconds for 20 candidates parameter settings
结果中的排名 : 1
平均validation score:0.800 (标准差: 0.017)
参数最终的优化的值: {'bootstrap': False, 'criterion': 'gini', 'max_depth': None, 'max_features': 9, 'min_samples_leaf': 5, 'min_samples_split': 8}
结果中的排名 : 2
平均validation score:0.800 (标准差: 0.017)
参数最终的优化的值: {'bootstrap': False, 'criterion': 'gini', 'max_depth': None, 'max_features': 9, 'min_samples_leaf': 5, 'min_samples_split': 8}
结果中的排名 : 3
平均validation score:0.800 (标准差: 0.017)
参数最终的优化的值: {'bootstrap': False, 'criterion': 'gini', 'max_depth': None, 'max_features': 9, 'min_samples_leaf': 5, 'min_samples_split': 8}
 最好优化值 {'bootstrap': False, 'criterion': 'entropy', 'max_depth': None, 'max_features': 8, 'min_samples_leaf': 1, 'min_samples_split': 2}
 """


randomized_search_cv(your_db_path)  # 随机采样参数优化
