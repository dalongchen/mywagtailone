...
class KzzStockPrice(object):
    ...
    @staticmethod  # 东财，输入代码获取股票k线数据 前复权, save == "y":  # 是否保存
    def east_history_k_data(code, save=''):
        import requests
        import demjson
        net = r"""http://push2his.eastmoney.com/api/qt/stock/kline/get
                ?fields1=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61&beg=0&end=20500101&ut=fa5fd1943c7b386f172d6893dbfba10b&rtntype=6&secid=1.{}&klt=101&fqt=1&cb="""
        dragon_t = requests.get(net.format(code)).text
        ...
        if dragon_t:
            ...
            if save == "y":  # 是否保存
                ...
                with sqlite3.connect(gl_v.db_path) as conn:
                    dragon_t.to_sql(code, con=conn, if_exists='replace', index=False)

stock = KzzStockPrice()
# stock.east_history_k_data('603368', save='y')
