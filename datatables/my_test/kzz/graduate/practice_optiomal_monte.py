...
# 实证最优蒙特卡洛
class PracticeOptionsMC(object):
    def __init__(self):
        pass
    ...
    @staticmethod  # 模拟的股价和收益率直方图
    def price_rate_hist(arr_mc, num):
        import matplotlib.pyplot as plt
        import matplotlib as mpl
        # matplotlib其实是不支持显示中文的 显示中文需要一行代码设置字体
        mpl.rcParams['font.family'] = 'SimHei'
        ...
        plt.xlabel('Time step')
        plt.ylabel('index level')
        """bins:可以是int也可以是一个序列，如果为一个整数，那么就会将数据范围分为n等分，如果是一个序列那么，就会按照序列进行分割"""
        # di = np.diff(arr_mc[num], n=1, axis=-1)
        # di2 = arr_mc[num, :-1]
        # di3 = di/di2
        logreturns = np.diff(np.log(arr_mc[num]))  #计算对数收益率
        plt.hist(logreturns, bins=50)  # 收益率直方图
        # plt.hist(arr_mc[500], bins=50)  # 股价直方图
        # plt.plot(logreturns, lw=0.5)  # 收益率震荡图, lw：折线图的线条宽度
        plt.title('模拟的第{}条股价路径收益率直方图 '.format(num))
        plt.show()
    ...
    def kzz_mc(self, s0, k, v, r, m, i, y_day, term, is_include):
        ...
        # self.price_rate_hist(arr_mc, 700)   # 模拟的股价和收益率直方图
        len_s0 = len(s0)
    ...
    def sql_for_day_value_mc(self, b_code, i, k, r, row_b, row_s, term, y_day):
        ...
        if len_b == row_s.shape[0]:
            ...
            for ii in range(len_b):
                ...
                dd = 'dd'
                if dd:
                    self.update_kzz_value_optimal_mc(b_code, kzz_value, r_b[0])
                time_end = time.clock()  # 记录结束时间
                time_sum = time_end - time_start  # 计算的时间差为程序的执行时间，单位为秒/s
                t_total += time_sum
                print(r_day, ii, '/总时:', (t_total / 60), '/单程:', time_sum, '/债值:', kzz_value, '/行权价:', k, '/净资产:', term['down'][2])
...
practice = PracticeOptionsMC()
# practice.sql_monte_carlo_call_option_value(s_code='603233', b_code='sh113605')  # 大参  10*len数据库版
practice.sql_monte_carlo_call_option_value(s_code='603368', b_code='sh113563')  # 柳药
